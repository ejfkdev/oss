...
const (
	findExists   = "exists"   // bucket exists (public or private)
	findNotFound = "notfound" // 404 / NXDOMAIN: bucket does not exist
	findUnknown  = "unknown"  // timeout / unexpected status: inconclusive
)
...
func findCmd() *cli.Command {
	flags := append([]cli.Flag{
		...
		&cli.BoolFlag{Name: "json", Aliases: []string{"j"}, Usage: T("NDJSON 输出（每个厂商一行 + 结果汇总行）", "NDJSON output (one line per provider + a summary line)")},
	}, connFlags()...)
	return &cli.Command{
		Name:    "find",
		Aliases: []string{"which"},
		Usage:   T("查找桶名存在于哪些云存储服务", "find which cloud storage services host a bucket name"),
		UsageText: T(`oss find <桶名> [--region <区域>]
...
命中时会输出完整访问 URL 列表，以及可直接运行的 oss 命令。
...
说明:
   ...
   - 探测为匿名请求，不发送任何凭证`,
			`oss find <bucket-name> [--region <region>]
...
When found, the full access URLs are printed together with ready-to-run
oss commands.
...
func runFind(ctx context.Context, c *cli.Command) error {
	...
	if bucket == "" {
		return errors.New(T("用法: oss find <桶名>", "usage: oss find <bucket-name>"))
	}
	...
	for _, p := range s3x.ScanProbes {
		v := verdict{probe: p, state: findNotFound}
		for _, r := range results {
			if r.Provider != p.Provider {
				continue
			}
			if r.State == findExists {
	...
	if c.Bool("json") {
		...
		// Summary line with the full access URLs of everything found.
		var foundItems []map[string]any
		for _, v := range verdicts {
			for _, m := range v.matches {
				foundItems = append(foundItems, map[string]any{
					"provider": v.probe.Provider,
					"name":     v.probe.Name,
					"region":   m.Region,
					"detail":   m.Detail,
		...
		line, _ := sonic.Marshal(map[string]any{"bucket": bucket, "found": foundItems})
		fmt.Println(string(line))
		return nil
	}
	...
	for _, v := range verdicts {
		...
		case findExists:
			...
			for _, m := range v.matches {
				foundMatches = append(foundMatches, foundMatch{name: v.probe.Name, m: m})
			}
		case findUnknown:
			mark = cYellow("?")
	...
	if len(found) > 0 {
		fmt.Printf("%s %s%s\n", cGreen("✓"),
			...
			strings.Join(found, ", "))
		fmt.Println()
		fmt.Println(cBold(T("访问 URL:", "Access URLs:")))
		for _, fm := range foundMatches {
			u := accessURL(fm.m.URL)
			label := fm.name
			if fm.m.Region != "" {
				label += " @ " + fm.m.Region
			}
			fmt.Printf("  %s %s\n", cCyan(label), cDim("("+fm.m.Detail+")"))
			fmt.Printf("    %s\n", u)
			fmt.Printf("    %s %s\n", cDim(T("→ 可直接使用:", "→ ready to use:")),
				fmt.Sprintf("oss ls %q", u+"?delimiter=/"))
		}
	} else {
		fmt.Printf("%s %s\n", cDim("✗"),
...
// accessURL returns the bucket access URL with a trailing slash.
func accessURL(u string) string {
...
func probeBucket(ctx context.Context, hc *http.Client, url string) (status int, state, detail string) {
	...
	if err != nil {
		return 0, findUnknown, err.Error()
	}
	resp, err := hc.Do(req)
	if err != nil {
		var dnsErr *net.DNSError
		if errors.As(err, &dnsErr) {
			return 0, findNotFound, T("域名不存在", "no such host")
		}
		if os.IsTimeout(err) || strings.Contains(err.Error(), "deadline") {
			return 0, findUnknown, T("超时", "timeout")
		}
		return 0, findUnknown, T("连接失败", "connection failed")
	}
	...
	case resp.StatusCode == http.StatusOK || resp.StatusCode == http.StatusNoContent:
		return resp.StatusCode, findExists, T("公开可读", "public")
	case resp.StatusCode >= 300 && resp.StatusCode < 400:
		return resp.StatusCode, findExists, T("重定向", "redirect")
	case resp.StatusCode == http.StatusUnauthorized || resp.StatusCode == http.StatusForbidden:
		return resp.StatusCode, findExists, T("私有", "private")
	case resp.StatusCode == http.StatusNotFound:
		return resp.StatusCode, findNotFound, T("桶不存在", "no such bucket")
	default:
		return resp.StatusCode, findUnknown, fmt.Sprintf("HTTP %d", resp.StatusCode)
	}
}
